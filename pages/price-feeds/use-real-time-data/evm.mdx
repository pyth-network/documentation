...
2. Call `IPyth.getUpdateFee` to calculate the fee charged by Pyth to update the price.
3. Call `IPyth.updatePriceFeeds` to update the price, paying the fee calculated in the previous step.
4. Call `IPyth.getPrice` to read the current price, providing the price feed ID that you wish to read.

Steps (2) and (3) both require a `bytes[] calldata priceUpdateData` argument that will be provided from a off-chain service: [Hermes](../pythnet-price-feeds/hermes.mdx).
...
2. Get price update data using this connection -- you can either perform one-off queries for this using `getPriceFeedsUpdateData` or subscribe for real-time updates via websocket.
3. Optionally get the Pyth fee required to perform the price update.
   You will need to do this step if your app is passing Pyth fees on to end users.
4. Pass the price update data to your contract.
