...
Consult: [Aptos Contract Addresses](../contract-addresses/aptos) for the complete list of contract addresses on Aptos.
...
module example::example {
    ...
    // Fetch the pyth_update_data from Hermes and pass it to the Pyth contract to update the prices.
    // Add the pyth_update_data argument to any method on your contract that needs to read the Pyth price.
    // See https://docs.pyth.network/price-feeds/fetch-price-updates for more information on how to fetch the pyth_update_data.
...
<Callout type="info" emoji="ℹ️">
  The `pyth_update_data` argument is required to update the Pyth price feeds.
  This data contains the price updates fetched from Hermes and passed to the
  Pyth contract to update the prices. Consult [Fetch Price
...
1. Call `pyth::update_price_feeds` and pass `pyth_update_data` to update the Pyth price feeds.
