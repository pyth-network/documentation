...
<Callout type="info" emoji="ℹ️">
  **Note**: Use an account address check to validate the price feed ID, if a
  user wants to integrate using the price feed accounts (see below).
</Callout>
...
**NOTE:** The Price Feed Accounts integration assumes that an off-chain process is continuously updating each price feed.
...
Running the scheduler can help with reliability and update feed/shard pairs that are not part of the default schedule.
...
// Send the instructions to the builder in 1 or more transactions.
