...
#### EVM revert getPrice() with `StalePrice()` or `0x19abf40e` error
This error occurs when the price data of the requested asset is not updated in the last valid time period.
...
NOTE: `getPriceUnsafe()` method does not check the freshness of the price data.


#### EVM revert getPrice() with `PriceFeedNotFound()` or `0x14aebe68` error

This error occurs when the requested price is not published on the respective chain or the prices have never been updated.

To resolve this issue:

- Update the price data by calling [`updatePriceFeeds()`]("https://github.com/pyth-network/pyth-crosschain/blob/main/target_chains/ethereum/sdk/solidity/IPyth.sol#L112") 
by passing the latest updateData from [Hermes](https://hermes.pyth.network/docs/#/rest/latest_vaas).
...
#### EVM revert updatePriceFeeds() with `InsufficientFee()` or `0x025dbdd4` error
