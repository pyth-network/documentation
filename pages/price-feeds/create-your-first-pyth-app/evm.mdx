...
In this tutorial we will develop a Solidity smart contract application that mints an NFT in exchange for $1 of ETH.
We will use Pyth realtime price feeds to determine the current price of ETH/USD. This turotial is divided into two parts: \
Part 1: Develop the contract using [pyth-sdk-solidity](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/sdk/solidity) and test it locally. \
Part 2: Deploy the contract to a test network and integrating [pyth-evm-js](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/sdk/js) to get the price updates.


## Preliminaries (TODO:)

In this tutorial we will uses Foundry to perform the contract development tasks.
...
Create a new directort and initialize a foundry project.

```bash copy
mkdir my_first_pyth_app
cd my_first_pyth_app
forge init
```
...
(TODO: Check if I can change the format of output block.)
```
[⠢] Compiling...
...
Pyth provides a Solidity SDK that we can use to interact with the on-chain contracts. 
It exposes multiple [methods](../api-reference/evm) to read and interact with the price feeds contract. 
You can use `npm` to install the Pyth SDK:
...
Next we will map the Pyth SDK so that we can import it from our Solidity contracts.
```bash copy
...
Next, create a file `src/MyFirstPythContract.sol` in your favorite editor and add the following code:
...
### Fetch the Price
Here we import the `IPyth` interface from the Pyth SDK and define a contract called `MyFirstPythContract`.
This interface is the primary way to interact with the Pyth price feeds contract. 

We pass the [Pyth's implementation contract address](../contract-addresses/evm) of along with [price feed id](https://pyth.network/developers/price-feed-ids) for `ETH/USD`. 


Next we will add a `mint` method which will do the following things:
1. Read the price of ETH/USD from Pyth.
2. Calculate the amount of ETH required to mint an NFT.
3. Check if the caller has paid enough ETH.
4. Mint the NFT if the caller has paid enough.
...
This method is reading the `Price` from the pyth contract.
We perfomed a conversion assuming that 10^18 wei is equal to the native token (ETH in this example); 
in some networks (like Hedera) the decimal places are different and you need to change the math.
If the caller has not paid enough, the function reverts.

### Update the Price with real time data

Now we will add the `update` function to the contract.
This function will update the price of ETH/USD on the Pyth contract and then call the `mint` function.
This method differentiate [pull oracles](https://docs.pyth.network/price-feeds/pythnet-price-feeds/pull-updates) from traditional push oracles. This allows users to fetch real-time data on demand.
This `pythPriceUpdate` data can be fetched from [Hermes](https://docs.pyth.network/price-feeds/pythnet-price-feeds/hermes)
TODO: Add reference to Part 2 here for consuming hermes data
...
This method calls `updatePriceFeeds` on the Pyth contract.
It take the payload `bytes[]` that we will fetch from `hermes` use it to update the latest prices on the contracts.
...
The caller of this function can pass in a recent Pyth price update as this payload, guaranteeing to fetch real-time data.
...
Let's try out our changes by running `forge build`:
...
[pyth-sdk-solidity](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/sdk/solidity) gives users [`Mock`](https://github.com/pyth-network/pyth-crosschain/blob/main/target_chains/ethereum/sdk/solidity/MockPyth.sol) methods to write unit testnetChains.
Create your test file in the `test` directory.
```bash copy
touch test/MyFirstPythContract.t.sol
```
...
Method `testMint` and `testMintRevert` set the price to a specific value using `mock` methods and then call `mint`.
The tests use a mock implementation of Pyth defined in helper merhods `createEthUpdate` and `setEthPrice`.
