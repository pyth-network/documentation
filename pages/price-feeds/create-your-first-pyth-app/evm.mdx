...
This tutorial is intended to help you get familiar with the pull-based oracles and the Pyth SDK.

In this two part tutorial, we will:
- Create a contract integrating [pyth-sdk-solidity](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/sdk/solidity) and test it locally.
- Deploy the contract to a test network and integrating [pyth-evm-js](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/sdk/js) to get the price updates.
...
```bash copy
mkdir my_first_pyth_app && cd my_first_pyth_app
forge init
```
...
(TODO: We can remove `forge test` here)
...
Now we are all set to start building our isConstructSignatureDeclaration..



## Install the Pyth SDK

Pyth provides a Solidity SDK that we can use to interact with the on-chain Pyth price-feed contracts. 
...
Now we will map the Pyth SDK so that we can import it from our Solidity contracts.
```bash copy
...
We pass the [Pyth's contract address](../contract-addresses/evm) along with [price feed id](https://pyth.network/developers/price-feed-ids) for `ETH/USD`. 


Now we will add a `mint` method to our contract which will do the following things:
...
contract MyFirstPythContract {
  ...
  function mint() public payable {
    ...
    if (msg.value >= oneDollarInEth) {
      // TODO: mint your favourite NFT here
    } else {
      revert ("Insufficient payment");
    }
  }
...
You have successfully fetched real time price data from Pyth Oracle. 🥳
...
Here methods `testMint` and `testMintRevert` set the price to a specific value using `mock` methods and then call `mint`.
The tests use a mock implementation of Pyth defined in helper methods `createEthUpdate` and `setEthPrice`.
...
[`skip`](https://book.getfoundry.sh/reference/forge-std/skip) moves the timestamp on the blockchain forward, which triggers the error.
...
To fix the `StalePrice` error, we will call `updateAndMint` instead of `mint`:


Now we will add the `updatePriceFeeds` function to out `MyFirstPythContract` contract.
...
```
This method will update the price of ETH/USD on the Pyth contract and call the `mint` function defined before.
This differentiate [pull oracles](https://docs.pyth.network/price-feeds/pythnet-price-feeds/pull-updates) from traditional push oracles. This allows users to fetch real-time data on demand.
...
We can test this function by adding the following snippet to the test file `MyFirstPythContract.t.sol` :
