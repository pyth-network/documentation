...
In this part of the tutorial, we will learn to deploy the contract and fetch the latest price from Pyth network.
...
This script defines the `abi` which represents the interface of our contract and can be autogenerated by running `forge inspect MyFirstPythContract abi` in the `contracts` directory.
...
In this 2 part tutorial, we created a Solidity contract that updates and reads Pyth prices, tested the contract locally,
then deployed the contract and interacted with it both via the command line and `pyth-evm-js` sdk.

## Next Steps

We recommend checking out [EVM Contract References](https://docs.pyth.network/price-feeds/api-reference/evm) for detailed information on the Pyth EVM contract API.
For and end to end example of using Pyth price feed, check out the [Pyth Oracle AMM Example](https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/ethereum/examples/oracle_swap).
