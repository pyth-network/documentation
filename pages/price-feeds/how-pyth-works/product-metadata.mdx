...
- `weekly_schedule` - Optional field. When set, contents are used by publishers to learn about a symbol's typical market hours. See [Schedule Format](schedule-format) for a detailed format specification.
