...
| <Arg required={true} type="bytes32[]">priceIds</Arg>       | <Input id="priceIds" format={InputFormats.ZeroX} />                 | The price ids to update                                                                                                                                                                                          |
| <Arg required={true} type="uint64[]">publishTimes</Arg>                | <Input id="publishTimes" format={InputFormats.BigInt} />                  | The timestamp for each price id that determines whether to apply the update.                                                                                                                                                               |
...
<Example
    keyValues={{
        updateData: async ctx => (await ctx.getLatestPriceFeed("Crypto.BTC/USD")).vaa,
        priceIds: (ctx) => ctx.getFeedId("Crypto.BTC/USD"),
        publishTimes: async ctx => (await ctx.getLatestPriceFeed("Crypto.BTC/USD")).price.publish_time,
        fee: async ctx => {
...
<EvmSend
    ...
    buildArguments={(kvs) => [[kvs["updateData"]], [kvs["priceIds"]], [kvs["publishTimes"]]]}
    feeKey="fee"
/>
...
<Tabs items={['Solidity', 'ethers.js v6']}>
    <Tab>
        <DynamicCode targets={{
            ...
            "0x1111111111111111111111111111111111111111111111111111111111111111": (ctx) => ctx.get("priceIds", "<priceId>"),
            "// NETWORK_NAME": (ctx) => `// ${ctx.evmConfig.info.chainName}`,
            "\"11\"": (ctx) => `"${ctx.get("updateData", "<updateData>").replace("0x", "")}"`,
            "2": (ctx) => ctx.get("fee", "<fee>"),
            "3": (ctx) => ctx.get("publishTimes", "<publishTime>"),
        }}>
            ```solidity
            ...
            bytes32[] priceIds = [0x1111111111111111111111111111111111111111111111111111111111111111];
            uint64[] publishTimes = [3];
            uint fee = 2;
    ...
    <Tab>
        <DynamicCode targets={{
            ...
            "'0x2222'": (ctx) => `'${ctx.get("priceIds", "<priceId>")}'`,
            "3": (ctx) => ctx.get("publishTimes", "<publishTime>"),
            "2": (ctx) => ctx.get("fee", "<fee>"),
        }}>
            ```typescript
