...
| <Arg required={true} type="uint64[]">minPublishTimes</Arg> | <Input id="minPublishTime" format={InputFormats.BigInt} /> | The minimum timestamp for each price id.                                 |
| <Arg required={true} type="uint64[]">maxPublishTimes</Arg> | <Input id="maxPublishTime" format={InputFormats.BigInt} /> | The maximum timestamp for each price id.                                 |
...
<Tabs items={['Solidity', 'ethers.js v6']}>
    <Tab>
        <DynamicCode targets={{
            ...
            uint64[] minPublishTimes = [3];
            uint64[] maxPublishTimes = [4];
            uint fee = 2;
            pyth.parsePriceFeedUpdates{value: fee}(updateData, priceIds, minPublishTimes, maxPublishTimes);
            ```
        </DynamicCode>
    </Tab>
    ...
    <Tab>
        <DynamicCode targets={{
            ...
            const minPublishTimes = [ethers.toBigInt(3)];
            const maxPublishTimes = [ethers.toBigInt(4)];
            const tx = await contract.parsePriceFeedUpdates(updateData, priceIds, minPublishTimes, maxPublishTimes, {value: fee});
