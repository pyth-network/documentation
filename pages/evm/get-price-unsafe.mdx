...
<Tab>
  ...
  >
    ```typescript const contractAddress = '0x0000'; const priceId = '0x1111';
    const PythAbi = require('./abis/IPyth.json'); const contract = new
    ethers.Contract(contractAddress, PythAbi, provider); const [price, conf,
    expo, timestamp] = await contract.getPriceUnsafe(priceId); ```
  </DynamicCode>
</Tab>
