...
The returned number of the specified token should be sent as the transaction value when executing [UpdatePriceFeeds](update-price-feeds).
...
| -------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------- |
| <Arg required={true} type="vec<Binary>">vaas</Arg> | <Input id="vaas" format={InputFormats.Base64}/> | The price updates that you would like to submit to [UpdatePriceFeeds](update-price-feeds) |
