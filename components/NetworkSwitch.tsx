...
const ClusterSwitch = ({
  ...
  if (options) {
    // for each option, check if its the value of any of the clusters
    // if it is keep it in the array, if not remove it
    const filteredClusters = clusters.filter((cluster) => {
      return options.includes(cluster.value)
    })
    // set clusters array to the filtered array
    clusters.length = 0
